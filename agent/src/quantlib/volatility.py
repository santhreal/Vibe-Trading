...
via numerical quadrature of the characteristic function (Albrecher et al. 2007 formulation).
...
def heston_characteristic_function(
    ...
    """Evaluate the Heston characteristic function phi_j(u) for log(S_T).

    j=1 gives the measure under stock numeraire (for P1);
    j=2 gives the risk-neutral measure (for P2).
    """
    i = 1j
    x0 = math.log(S0)
    sigma_sq = sigma_v**2
    ...
    d = cmath.sqrt((rho * sigma_v * i * u - b_j)**2 - sigma_sq * (2.0 * u_j * i * u - u**2))
    g = (b_j - rho * sigma_v * i * u + d) / (b_j - rho * sigma_v * i * u - d)

    exp_dt = cmath.exp(d * T)
    one_minus_g_exp = (1.0 - g * exp_dt) / (1.0 - g)

    C = (r - q) * i * u * T + (kappa * theta / sigma_sq) * (
        (b_j - rho * sigma_v * i * u + d) * T - 2.0 * cmath.log(one_minus_g_exp)
    )

    D = ((b_j - rho * sigma_v * i * u + d) / sigma_sq) * ((1.0 - exp_dt) / (1.0 - g * exp_dt))
...
def heston_price(
    ...
    def p_integrand(u: float, j: int) -> float:
        if u == 0.0:
            return 0.0
        phi = heston_characteristic_function(
            u, S0, T, r, q, v0, kappa, theta, sigma_v, rho, j=j
        )
        val = (cmath.exp(-1j * u * k) * phi) / (1j * u)
        return float(val.real)

    int1, _ = quad(lambda u: p_integrand(u, 1), 0.0, integration_limit, limit=200)
    int2, _ = quad(lambda u: p_integrand(u, 2), 0.0, integration_limit, limit=200)
    ...
    call = float(discounted_F * P1 - discounted_K * P2)
